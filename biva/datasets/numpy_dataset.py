...
def get_transforms_2d(normalise, target_size=128):
    ...
    """
    if normalise:
        transforms = Compose([Resize((target_size, target_size)), Lambda(lambda x: x * 2 - 1), ToTensor()])
    else:
        transforms = Compose([Resize((target_size, target_size)), ToTensor()])
    return transforms


def get_transforms_3d(normalise, target_size=128):
...
def get_numpy2d_dataset(
    ...
    transforms = get_transforms_2d(normalise=normalise, target_size=target_size)
...
def get_numpy3d_dataset(
    ...
    transforms = get_transforms_3d(normalise=normalise, target_size=target_size)
...
class Numpy2dDataSet(Dataset):
    ...
    def get_data_by_idx(self, idx):
        ...
        del numpy_array
        return numpy_slice.astype(np.float32)/np.max(numpy_slice.astype(np.float32)+1e-8)
    ...
    def load_dataset(self, base_dir, mode="train", pattern="*data.npy", slice_offset=0, caching=True):
        ...
        all_files = os.listdir(base_dir)[:100]
